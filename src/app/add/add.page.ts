...
export class AddPage implements OnInit {
    ...
    submitAddNote() {
        ...
        this.storage.get('allNotes').then((res) => {
            let tag = new Date(this.addNote.dateTime).getDate();
            let wochentagZahl = new Date(this.addNote.dateTime).getDay();
            let wochentagWordArray = ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'];
            let wochentagWord = wochentagWordArray[wochentagZahl];
            let monat = new Date(this.addNote.dateTime).getMonth();
            let jahr = new Date(this.addNote.dateTime).getFullYear();
            let datum = wochentagWord + ', ' + tag + '.' + (monat + 1) + '.' + jahr;

            let minute = new Date(this.addNote.dateTime).getMinutes();
            let minute02;
            if (minute < 10) {
                minute02 = '0' + minute;
            } else {
            ...
            let stunde = new Date(this.addNote.dateTime).getHours();
            let stunde02;
            if (stunde < 10) {
                stunde02 = '0' + stunde
            } else {
